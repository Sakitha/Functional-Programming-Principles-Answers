...
trait FunSets extends FunSetsInterface {
  ...
  //def contains(x => x == 1, elem: Int): Boolean = s(elem)
  // elem == 1
  /**
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = x => contains(p,x) && contains(s,x)
  ...
  //def contains(map(x => x > 4, x => x * 2), 2 )
  //def contains(x => x == 2, 2 )
  // elem == 1
  def map(s: FunSet, f: Int => Int): FunSet = x => exists(s, y => y == f(x))
